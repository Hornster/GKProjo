...
public class CastRays
{
    ...
    public (List<LaserPartData>, Vector2) ProjectRectangle(RectangleF objectDimensions, Vector2 direction, double distance,
        LayerMask checkedLayers, int additionalHorizontalRays = 3, int additionalVerticalRays = 2)
    {
        ...
        do
        {
            ...
            (closestResultCast, closestResultOrigin) = CastMultipleRays(direction, distance, checkedLayers);
    ...
    public List<LaserPartData> GetLaserPath(Vector2 startingPoint, Vector2 direction, double distance, LayerMask checkedLayers)
    {
        ...
        do
        {
            ...
            var resultCast = PerformCastRay(startingPoint, direction, distance, checkedLayers);//Perform raycast
    ...
    private RaycastHit2D PerformCastRay(Vector2 startingPoint, Vector2 direction, double distance, LayerMask checkedLayers)
    {
        var raycastHit = Physics2D.Raycast(startingPoint, direction, (float)distance, checkedLayers);
        return raycastHit;
    }
    ...
    /// <returns></returns>
    private (RaycastHit2D, Vector2) CastMultipleRays(Vector2 direction, double distance, LayerMask checkedLayers)
    {
        ...
        {
            (closestHit, lastHit, closestHitRayOrigin) = CastAndTestSingleRay(ref closestHit, rayOrigin, ref closestHitRayOrigin, ref direction, (float)distance, checkedLayers);
    ...
    private (RaycastHit2D, RaycastHit2D, Vector2) CastAndTestSingleRay(ref RaycastHit2D currentClosestHit, Vector2 rayOrigin, ref Vector2 currentClosestHitRayOrigin,
        ref Vector2 direction, float distance, LayerMask checkedLayers)
    {
        var raycastHit = Physics2D.Raycast(rayOrigin, direction, (float)distance, checkedLayers);
